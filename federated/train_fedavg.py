...
def main():
    ...
    logger = setup_logging(log_dir, log_filename="train_fedavg.log")
    
    logger.info("=== Initialize Federated Learning (FedAvg) Training ===")
    ...
    feature_cols = config['dataset']['num_features'] # placeholder or loaded
    ...
    # Combines validation split to verify global model performance
    ...
    # 9. Execute Federated Training Loop (Rounds)
    ...
    logger.info("Generating and saving federated training curves...")
    FederatedEvaluator.plot_rounds_vs_performance(history, results_dir)
    logger.info(f"Saved rounds vs. loss/auroc curves to: {results_dir}")
