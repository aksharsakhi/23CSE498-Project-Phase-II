...
\author{
\IEEEauthorblockN{Sheela Akshar Sakhi, Hasini Reddy M, Kousik Sarma Lakkaraju, Haswitha K, and V. Chakravarthy}
\IEEEauthorblockA{\textit{Department of Computer Science and Engineering, Amrita School of Computing, Coimbatore} \\
\textit{Amrita Vishwa Vidyapeetham, India} \\
\{cb.sc.u4cse23547, cb.sc.u4cse23529, cb.sc.u4cse23761, cb.sc.u4cse23363, cb.sc.u4cse23753\}@cb.amrita.edu}
}
...
To bridge all three gaps, we propose the \textbf{Federated Personalized Drift-Aware Attention Framework (FPDAF)}. The key contributions of this paper are:
\begin{itemize}
...
\begin{figure*}[t]
\centering
\subfloat[Imputation \& Z-Score Normalization]{\includegraphics[width=0.48\textwidth]{images/preprocessing_comparison.png}\label{fig:preproc_comp}}
\hfill
\subfloat[Zero Pre-Padding Validation ($T < 24$h)]{\includegraphics[width=0.48\textwidth]{images/preprocessing_padding.png}\label{fig:preproc_pad}}
...
\begin{figure*}[t]
\centering
\subfloat[Centralized Test ROC Curve]{\includegraphics[width=0.48\textwidth]{images/baseline_roc.png}\label{fig:base_roc}}
\hfill
\subfloat[Test Confusion Matrix Heatmap]{\includegraphics[width=0.48\textwidth]{images/baseline_confusion_matrix.png}\label{fig:base_cm}}
...
\begin{itemize}
\item \textbf{Hospital 1 (Client Node 0)}: 92,613 training sequence samples, 3.24\% positive rate.
\item \textbf{Hospital 2 (Client Node 1)}: 151,916 training sequence samples, 3.09\% positive rate.
\item \textbf{Hospital 3 (Client Node 2)}: 233,140 training sequence samples, 2.07\% positive rate.
\end{itemize}
...
\begin{figure*}[t]
\centering
\subfloat[Validation Loss Across Rounds]{\includegraphics[width=0.48\textwidth]{images/federated_rounds_loss.png}\label{fig:fed_loss}}
\hfill
\subfloat[Validation AUROC Across Rounds]{\includegraphics[width=0.48\textwidth]{images/federated_rounds_auroc.png}\label{fig:fed_auroc}}
...
Due to client Non-IID heterogeneity, FedAvg exhibits a \textbf{2.14\% drop in test AUROC} (0.7844 vs 0.8058) and a \textbf{4.97\% drop in Recall}, confirming that a single static global consensus model struggles under institutional demographic shifts. Figure~\ref{fig:fed_roc_cm} plots the FedAvg ROC curve and confusion matrix.

\begin{figure*}[t]
\centering
\subfloat[FedAvg Test ROC Overlay]{\includegraphics[width=0.48\textwidth]{images/federated_comparative_roc.png}\label{fig:fed_roc}}
\hfill
\subfloat[FedAvg Confusion Matrix]{\includegraphics[width=0.48\textwidth]{images/federated_confusion_matrix.png}\label{fig:fed_cm}}
...
\begin{figure*}[t]
\centering
\subfloat[FedProx Validation Loss]{\includegraphics[width=0.48\textwidth]{images/fedprox_rounds_loss.png}\label{fig:prox_loss}}
\hfill
\subfloat[FedProx Validation AUROC]{\includegraphics[width=0.48\textwidth]{images/fedprox_rounds_auroc.png}\label{fig:prox_auroc}}
...
FedProx improves test AUROC to \textbf{0.8033} (+1.89\% over FedAvg) and increases F1-score to \textbf{0.1622} (+3.63\% over FedAvg). Figure~\ref{fig:fedprox_roc_cm} displays the three-way ROC curve and FedProx confusion matrix.

\begin{figure*}[t]
\centering
\subfloat[Three-Way ROC Overlay]{\includegraphics[width=0.48\textwidth]{images/fedprox_three_way_roc.png}\label{fig:prox_roc}}
\hfill
\subfloat[FedProx Confusion Matrix]{\includegraphics[width=0.48\textwidth]{images/fedprox_confusion_matrix.png}\label{fig:prox_cm}}
...
Table~\ref{tab:fourway} summarizes four-way performance. Ditto Personalized achieves an Accuracy of \textbf{86.01\%} (+10.07\% over FedAvg) and F1-score of \textbf{0.1629}. Figure~\ref{fig:ditto_roc_cm} plots four-way ROC overlays and confusion matrices.
...
\begin{figure*}[t]
\centering
\subfloat[Four-Way ROC Overlay]{\includegraphics[width=0.48\textwidth]{images/four_way_roc_curve.png}\label{fig:ditto_roc}}
\hfill
\subfloat[Ditto Personalized Confusion Matrix]{\includegraphics[width=0.48\textwidth]{images/ditto_personalized_confusion_matrix.png}\label{fig:ditto_cm}}
\caption{Four-way comparative test set evaluation.}
\label{fig:ditto_roc_cm}
\end{figure*}

\section{Proposed FPDAF Framework}
We propose the \textbf{Federated Personalized Drift-Aware Attention Framework (FPDAF)}, integrating multi-head temporal self-attention, AD-CUSUM drift detection, and DDP-ERP personalization.
...
To track institutional population drift online without false alarms from mini-batch noise, we formulate \textbf{AD-CUSUM}:
...
To maximize precision under clinical cohort variance, we invent \textbf{DDP-ERP} for FPDAF-v2:
\begin{enumerate}
...
\begin{figure*}[t]
\centering
\subfloat[Five-Way ROC Overlay]{\includegraphics[width=0.48\textwidth]{images/five_way_roc_curve.png}\label{fig:five_roc}}
\hfill
\subfloat[Average Temporal Attention Curve]{\includegraphics[width=0.48\textwidth]{images/temporal_attention_profile.png}\label{fig:attn_prof}}
