...
To bridge all three gaps, we propose the \textbf{Federated Personalized Drift-Aware Attention Framework (FPDAF)}.
...
We evaluate our framework on the PhysioNet Computing in Cardiology Challenge 2019 dataset, comprising 40,327 ICU patient records with 40 hourly physiological signals (e.g., Heart Rate, Mean Arterial Pressure, Temperature, Respiratory Rate, Oxygen Saturation, WBC count) and hourly binary `SepsisLabel` annotations. Table~\ref{tab:vitals_schema_springer} details the primary physiological vital signs and clinical laboratory features monitored.

\begin{table}[htbp]
\caption{Physiological Vital Signs \& Laboratory Feature Schema}
\label{tab:vitals_schema_springer}
\centering
\small
...
\begin{figure}[htbp]
\centering
\subfloat[Imputation \& Z-Score Normalization]{\includegraphics[width=0.48\textwidth]{images/preprocessing_comparison.png}\label{fig:preproc_comp_sp}}
\hfill
\subfloat[Zero Pre-Padding Validation ($T < 24$h)]{\includegraphics[width=0.48\textwidth]{images/preprocessing_padding.png}\label{fig:preproc_pad_sp}}
\caption{Data preprocessing engineering pipeline validation.}
\label{fig:preprocessing_springer}
\end{figure}
...
\begin{figure}[htbp]
\centering
\subfloat[Five-Way ROC Overlay]{\includegraphics[width=0.48\textwidth]{images/five_way_roc_curve.png}\label{fig:five_roc_sp}}
\hfill
\subfloat[Temporal Attention Profile]{\includegraphics[width=0.48\textwidth]{images/temporal_attention_profile.png}\label{fig:attn_prof_sp}}
\caption{Clinical explainability and comparative performance overlays.}
\label{fig:clinical_attention_springer}
\end{figure}
