...
\begin{frame}{Core Technical Novelty \& Key Contributions}
    \begin{columns}
        \begin{column}{0.56\textwidth}
            \begin{block}{1. Multi-Head Attention Explainability}
                \scriptsize Combines recurrent sequences with query-key self-attentions to compute dynamic temporal saliency maps for bedside ICU interpretability.
            \end{block}
            \begin{block}{2. Online CUSUM Drift Monitoring}
                \scriptsize Monitors validation loss residuals locally ($S_r > 3.0$) to audit institutional shifts without constant server aggregations.
            \end{block}
            \begin{block}{3. Client-Side Selective Personalization (CSSP)}
                \scriptsize Freezes global model backbones upon drift triggers to train local classifier heads, completely bypassing server weight uploads.
            \end{block}
        \end{column}
        \begin{column}{0.44\textwidth}
            \begin{block}{Core Scientific Contributions}
                \scriptsize
                \begin{itemize}
                    \item \textbf{Bandwidth Savings:} CSSP cuts local-to-server bandwidth overhead by \textbf{38\%} vs. Ditto.
                    \item \textbf{Adaptation Speed:} Head-only fine-tuning runs in \textbf{6 minutes} (down from 25m).
                    \item \textbf{Diagnostic Saliency:} Sepsis risk triggers are mapped hourly for clinical compliance.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
