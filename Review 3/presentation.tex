...
\begin{frame}{Interactive Clinician Dashboard UI}
    \begin{columns}[t]
        \begin{column}{0.5\textwidth}
            \begin{block}{Doctor's Bedside Diagnostic View}
                \scriptsize
                Provides live diagnostics and explanations directly at the patient's bedside:
                \begin{itemize}
                    \item \textbf{Timeline Telemetry:} Plots hourly ICU vital segments (HR, SBP, Temp, Resp, SpO$_2$) using Recharts.
                    \item \textbf{Live Predictions Dials:} Computes and overlays risk probability scores for all federated configurations.
                    \item \textbf{Attention Saliency Maps:} Attributes predictions to specific stay intervals (e.g. onset hours 16--22).
                \end{itemize}
            \end{block}
        \end{column}
        
        \begin{column}{0.5\textwidth}
            \begin{block}{Admin's Federated Telemetry View}
                \scriptsize
                Provides real-time machine learning monitoring:
                \begin{itemize}
                    ...
                    \item \textbf{Design System:} Tailwind CSS v4 class-based toggling supporting light/dark medical navy modes.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
