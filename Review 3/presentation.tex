    ...
    \begin{columns}[t]
        \begin{column}{0.48\textwidth}
            \begin{block}{Reviewer Comments (Review 2)}
                ...
                \begin{itemize}
                    ...
                    \vspace{0.3cm}
                    \item \textbf{Comment 3: Concept Drift Detection} \\
        ...
        \begin{column}{0.52\textwidth}
            \begin{block}{Action Taken \& Implementation Status}
                \scriptsize
                \begin{itemize}
                    \item \textbf{Implemented Federated Pipelines (Comments 1 \& 3):}
                    \begin{itemize}
                        \scriptsize
                        \item Built decentralized PyTorch algorithms (FedAvg, FedProx).
                        \item Invented and integrated the \textbf{Dynamic Dual-Phase (DDP)} Cross-Attention algorithm.
                        \item Implemented local \textbf{AD-CUSUM drift detection} (Adaptive Divergence-Weighted CUSUM) to identify clinical shifts in real-time.
                    \end{itemize}
                    \vspace{0.2cm}
                    \item \textbf{Built Production-Grade CDSS Workstation (Comment 2):}
                    \begin{itemize}
                        \scriptsize
...
\begin{frame}{Core Project Inventions \& Research Novelties}
    \begin{columns}[t]
        ...
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{3. Dynamic Dual-Phase (DDP) Cross-Attention}
                \scriptsize
                Integrated 2D cross-attention pooling mechanism capturing 24-hour sequence time steps ($\alpha_t$) AND vital co-dependencies ($\beta_v$) to capture multi-modal interactions.
            \end{alertblock}
...
\begin{frame}{Clinical Deterioration \& Phase II Completed Goals (60\% Met)}
    \begin{columns}
        ...
        \begin{column}{0.45\textwidth}
            \begin{block}{Phase II Completed Milestones}
                ...
                \begin{itemize}
                    ...
                    \item \textbf{3. Personalized FPDAF Model:} Invented and trained the \textbf{Dynamic Dual-Phase (DDP)} cross-attention algorithm with localized personalization.
...
\begin{frame}{Model Taxonomy \& Technical Insights Extracted}
    ...
    \begin{block}{Methodological Evolution}
        \tiny
        By combining the decentralized privacy of FedAvg and drift stability of FedProx with our new \textbf{Dynamic Dual-Phase (DDP) Cross-Attention} and \textbf{AD-CUSUM mechanism}, FPDAF achieves superior predictive stability while strictly guaranteeing patient data privacy.
    \end{block}
\end{frame}
...
% --- CLINICIAN DASHBOARD: DATABASE SCHEMA ---
\begin{frame}{Clinical Data Warehouse \& Schema (`clinical\_warehouse.db`)}
    \begin{columns}
        \begin{column}{0.52\textwidth}
            \centering
            \includegraphics[width=\textwidth]{images/dashboard_drift.png}
            \vspace{0.1cm}
            \scriptsize \textbf{Figure 4:} Relational Schema and Active Database Telemetry Tables.
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Database Table Specifications}
                \scriptsize
                \begin{itemize}
                    \item \textbf{patients}: Stores bed IDs, demographics, and clinical sepsis status.
                    \item \textbf{vitals}: 24-hour sequence vitals decoded with \texttt{scaler.pkl}.
                    \item \textbf{predictions}: Model probability outputs across FedAvg/FedProx/FPDAF.
                    \item \textbf{attentions}: Temporal \& DDP cross-attention weights ($\alpha_t \otimes \beta_v$).
                    \item \textbf{drift\_metrics}: Real-time AD-CUSUM residuals ($S_r$) and CSSP flags.
                    \item \textbf{model\_comparison}: Benchmark metrics across all paradigms.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Decentralized Model Update Lifecycle (Future Implementation)}
    \begin{columns}[t]
        \begin{column}{0.5\textwidth}
            \begin{block}{Proposed 5-Step Iterative Communication Loop}
                \scriptsize
                \begin{enumerate}
...
\begin{frame}{Project Roadmap \& Future Work (60\% Project Objectives Met)}
    \scriptsize
    \begin{table}
        \centering
        ...
        \begin{tabular}{|l|l|c|}
            \hline
            \textbf{Project Phase} & \textbf{Key Milestones \& Tasks} & \textbf{Status} \\
            \hline
            \textbf{Phase I} & Literature Survey, Dataset Extraction, System Architecture Design & \textbf{\textcolor{green!60!black}{Completed}} \\
            \hline
            \textbf{Phase II (Review 3)} & Implemented AD-CUSUM Drift Detection \textit{(FPDAF-v1 Baseline)} & \textbf{\textcolor{green!60!black}{Completed}} \\
            \hline
            \textbf{Phase II (Review 3)} & Developed production-ready Full-Stack Clinician CDSS Dashboard & \textbf{\textcolor{green!60!black}{Completed}} \\
            \hline
            \textbf{Phase II (Review 3)} & Invented \textit{Dynamic Dual-Phase (DDP)} Cross-Attention Algorithm & \textbf{\textcolor{green!60!black}{Completed}} \\
            \hline
            \textbf{Future Work} & \textbf{Next Step:} Implement \textit{Entropy-Regularized Personalization (ERP)} & \textbf{\textcolor{blue}{Pending}} \\
            \hline
            \textbf{Future Work} & \textbf{Next Step:} Final 6-Way Benchmarking (aiming for $>95\%$ Accuracy) & \textbf{\textcolor{blue}{Pending}} \\
            \hline
            \textbf{Future Work} & \textbf{Next Step:} Full 3-Hospital Decentralized Loop Simulation & \textbf{\textcolor{blue}{Pending}} \\
            \hline
        \end{tabular}}
    \end{table}
    \vspace{0.2cm}
    \begin{block}{Roadmap Alignment}
        100\% of Review 3 objectives have been completed, representing \textbf{60\% completion of total project objectives}. Future work involves pushing predictive accuracy beyond the current baseline by formulating advanced multi-modal attention mechanisms.
    \end{block}
\end{frame}
...
\begin{frame}{Individual Team Contributions}
    ...
    \begin{table}
        ...
        \begin{tabular}{|l|l|p{7.5cm}|}
            ...
            \hline
            1 & Sheela Akshar Sakhi & Federated framework architecture design, Dynamic Dual-Phase (DDP) Cross-Attention, Centralized BiLSTM baselines, Full-Stack Clinician Dashboard integration. \\
            \hline
