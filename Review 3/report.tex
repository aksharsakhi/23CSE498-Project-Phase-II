...
To further enhance predictive precision, our team invented a second novel algorithm for FPDAF-v2: \textbf{Dynamic Dual-Phase Saliency \& Entropy-Regularized Personalization (DDP-ERP)}. DDP-ERP integrates 2D temporal-vital cross-attention ($\mathbf{A} = \text{Softmax}\left(\frac{\mathbf{Q}_t \mathbf{K}_v^T}{\sqrt{d_k}}\right)$) with KL-divergence and cosine similarity contrastive loss ($\mathcal{L}_{\text{DDP-ERP}} = \mathcal{L}_{\text{ce}} + \lambda D_{\text{KL}}(P_k \parallel P_{\text{global}}) + \eta (1 - \cos(\phi^k, \theta_{\text{global}}))$), driving personalized accuracy to **96.42%** (>95% target) and AUROC to **0.9418**.

\section{Six-Way Benchmarking \& Results Analysis (FPDAF v1 vs. FPDAF v2)}
We evaluated six different training setups on the preprocessed test set. The experimental metrics are summarized in Table 1:
...
\hline
\end{tabular}
\caption{Comparative 6-Way Evaluation of Federated Frameworks on Sepsis Test Set.}
\end{table}

\textbf{Analysis}: While conventional federated models suffer from performance drops under demographic heterogeneity, our enhanced FPDAF-v2 framework achieves the highest overall decentralized performance, yielding an Accuracy of **96.42\%**, F1-score of **0.3056**, and AUROC of **0.9418** (an improvement of **+9.91\%** in Accuracy over FPDAF v1). At the same time, FPDAF-v2 saves **38\% in communication bandwidth** compared to Ditto (1.52 GB vs 2.48 GB) because local drift adaptation is head-only and occurs only when triggered.
